...
var mana_cost: int = 0     # стоимость в мане; 0 — бесплатно. Серверная проверка — источник истины.
...
# Срабатывает после успешной отправки скилла на сервер. По умолчанию —
# no-op: анимации единообразно приходят через OP_PLAYER_ACTION с сервера.
# Переопределять только для чисто-клиентских эффектов (например shake
# экрана при касте), не для базовой анимации.
