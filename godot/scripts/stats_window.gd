...
func refresh(me: Dictionary) -> void:
	...
	_set(_char_lbl, "xp", "lv%d · %d / %d" % [lv, xp, xp_next] if xp_next > 0 else "lv%d · %d" % [lv, xp])
	var hp := int(me.get("hp", 0)); var hp_max := int(me.get("hpMax", 0))
	_set(_char_lbl, "hp", "%d / %d" % [hp, hp_max])
	_set(_char_lbl, "hp_regen", "—")
	_set(_char_lbl, "mp", "—")
	_set(_char_lbl, "mp_regen", "—")
	# Атака
	var phys: int = int(me.get("atk", me.get("attackDamage", 0)))
	_set(_atk_lbl, "phys_dmg", str(phys))
	_set(_atk_lbl, "mag_dmg", "—")
	_set(_atk_lbl, "crit_chance", "—")
	_set(_atk_lbl, "crit_power", "—")
	_set(_atk_lbl, "accuracy", "—")
	_set(_atk_lbl, "penetration", "—")
	_set(_atk_lbl, "cd_reduction", "—")
	_set(_atk_lbl, "stun", "—")
	_set(_atk_lbl, "pvp_bonus", "—")
	# Защита
	_set(_def_lbl, "phys_def", "—")
	_set(_def_lbl, "mag_def", "—")
	_set(_def_lbl, "dodge", "—")
	_set(_def_lbl, "pvp_resist", "—")
	_set(_def_lbl, "block", "—")
	_set(_def_lbl, "lifesteal", "—")
	_set(_def_lbl, "reflect", "—")

func _set(sink: Dictionary, key: String, value: String) -> void:
